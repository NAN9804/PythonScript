...
'''
vInst_gen.py -- generate verilog module Instance
generated instance file like this:
...
Usage:
      python vInst_gen.py ModuleFileName.v
...
def formatPort(AllPortList,isPortRange =1) :
    ...
    str =''
    if AllPortList !=[] :
        l1 = max([len(i[0]) for i in AllPortList])+2
        l3 = max(16, l1)

        str = ',\n'.join( [' '*4+'.'+ i[0].ljust(l3)
                            + '('+ (re.sub(r'[I_O_]','',i[0])) + ')' for i in AllPortList ] )
...
def formatDeclare(PortList,portArr, initial = "" ):
    ...
    if PortList!=[] :
        str = '\n'.join( [ portArr.ljust(4) +'  '+(i[1]+min(len(i[1]),1)*'  '
                           +(re.sub(r'[I_O_]','',i[0]))) + ';' for i in PortList])
    return str
...
def writeInstance(input_file):
    ...
    # extract text between moduel ... endmodule  #
    ...
    allTypePort = portDeclare(inText,r'(input | output | inout)')

    portList = formatPort( allTypePort )
